"""No-UK ensemble retrain (part 1): GPU-LGB x3 on base-222 + dip(5) + cwt(4) = 231 feats.

Identical to experiments/ens_train_lgb.py EXCEPT the 3 UK cols (tvt_uk_d, uk_ancc, uk_vs_dense) are
excluded from feature_cols. Reads the SAME frontier_ens parquets (which carry all 12 lever feats) so
this is a pure feature-subset comparison vs the 234-feat ens (LB 8.171, regressed) -- does dropping the
dominant-but-LB-regressing UK feat recover the joint signal?

Run (skynet, GPU-LGB): nohup python -u experiments/ens_nouk_train_lgb.py > log/ens_nouk_lgb_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
...
FR = ROOT / "data/processed/frontier_ens"          # reuse the 12-feat parquets; we just exclude UK
MODELS = ROOT / "models/frontier_ens_nouk"
...
print(">> loading frontier_ens matrices (excluding UK)...", flush=True)
train_df = pd.read_parquet(FR / "train_feats.parquet")
test_df = pd.read_parquet(FR / "test_feats.parquet")
feature_cols = [c for c in train_df.columns if c not in {"well", "id", "target"} and c not in UK]
assert not (UK & set(feature_cols)), "UK leaked into feature_cols"
print(f"   train {train_df.shape}  #feats={len(feature_cols)} (UK excluded: {sorted(UK)})", flush=True)
...
print(f"\n=== LGB-231 (frontier_ens NO-UK) summary ===", flush=True)
...
print("=== ENS NO-UK LGB DONE ===", flush=True)
