"""frontier_super retrain (part 2): GPU-CatBoost x3 on the union matrix (nouk-231 + 28 super cols).

Identical to experiments/ens_nouk_train_cat.py except it reads data/processed/frontier_super
(UK already excluded at build time) and writes models/frontier_super/. SAME folds/params/seeds.

Run (cluster job, pinned RTX 4080 / or runon deepthought):
  nohup python -u experiments/ens_super_train_cat.py > log/ens_super_cat_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
...
print(f"\n=== CatBoost frontier_super summary ===", flush=True)
for k, v in results.items():
    print(f"  {k}: {v:.4f}", flush=True)
json.dump(results, open(MODELS / "cat_summary.json", "w"), indent=2)
print("=== ENS SUPER CAT DONE ===", flush=True)
