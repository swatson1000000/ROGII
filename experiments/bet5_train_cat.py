"""Phase B (part 2): GPU-CatBoost x3 on the 222-feat frontier union (deepthought).

Uses the Optuna-tuned params (models/konbu/cat_hpo.json, depth7/lr0.040/l2 16.2; tuned on the
78-feat base, reasonable warm start for 222) across 3 seeds, same GKF-5 seed42 as LGB. Saves
OOF + test preds -> models/frontier/ to blend with the skynet LGB run.

Run (deepthought, GPU CatBoost): nohup python -u experiments/frontier_train_cat.py > log/frontier_cat_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
...
print(f">> train {X.shape}  #feats={len(feature_cols)}  (tuned CatBoost-78 solo 11.835)", flush=True)
...
print(f"\n=== CatBoost-222 summary ===", flush=True)
for k, v in results.items():
    print(f"  {k}: {v:.4f}", flush=True)
print(f"  (tuned CatBoost-78 solo 11.835; banked full stack 11.821)", flush=True)
json.dump(results, open(MODELS / "cat_summary.json", "w"), indent=2)
print("=== FRONTIER CAT DONE ===", flush=True)
