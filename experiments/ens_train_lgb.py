"""Ensemble retrain (part 1): GPU-LGB x3 on the 234-feat frontier_ens union (base-222 + 12 lever feats).

Identical harness/params/folds to experiments/bet5_train_lgb.py so frontier_ens vs frontier_uk vs
frontier_seeded is a pure feature-set comparison. The decisive number is the 6-model Ridge-stack OOF
(see ens_blend.py) vs banked frontier 8.158-stack OOF: does the single-LGB +ALL12 -0.128 joint signal
survive the full stack, or get absorbed/inverted like UK did?

Run (skynet, GPU-LGB): nohup python -u experiments/ens_train_lgb.py > log/ens_lgb_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
...
print(">> loading frontier_ens matrices...", flush=True)
...
print(f"\n=== LGB-234 (frontier_ens) summary ===", flush=True)
