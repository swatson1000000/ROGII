"""Super-solution re-blend (option a): LGB×3 + CatBoost×3 (6 models) + 3D postproc -> RE-GATE.

Identical protocol to super_blend.py but with the 6-model stack (adds cat_7, cat_123 from
super_train_cat_extra.py). Ridge(alpha=1, positive=True) over the 6 OOF, pick max(avg, ridge).
Raw blend is the honest gate vs frontier 10.356. Writes blend6_summary.json (leaves the original
4-model blend_summary.json untouched for provenance).
"""
...
# ---- 6-model Ridge(positive) blend; pick max(avg, ridge) per source ----
...
print(f"\n=== SUPER 6-MODEL GATE SUMMARY (vs frontier {FRONTIER}) ===", flush=True)
print(f"   raw 6-blend : {raw_rmse:.4f}   ({raw_rmse-FRONTIER:+.3f})", flush=True)
...
np.save(M / "test_blend6_drift.npy", test_drift)
json.dump({"keys": KEYS, "weights": w.tolist(), "use_ridge": bool(use_ridge),
           ...
           "sg": sg_rmse, "frontier_ref": FRONTIER, "verdict": verdict,
           "per_model": {k: float(np.sqrt(np.mean((oofs[k]-y)**2))) for k in KEYS}},
          open(M / "blend6_summary.json", "w"), indent=2)
print("=== SUPER BLEND6 DONE ===", flush=True)
