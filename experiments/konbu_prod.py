...
# ---------------- production main: parallel features + cache + GPU train + save ----------------
...
MODELS = Path("/home/swatson/work/kaggle/ROGII/models/konbu")
N_BUILD_WORKERS = 14

# module globals so forked workers inherit the (read-only) imputers
_FORM_IMP = None
_ROW_IMP = None


def _build_one(args):
    """Worker: build features for a single well. Uses fork-inherited imputers."""
    p, is_train = args
    wid = p.stem.replace("__horizontal_well", "")
    h = pd.read_csv(p)
    t = pd.read_csv(p.parent / f"{wid}__typewell.csv")
    if is_train and "TVT" not in h.columns:
        return None
    return build_hidden_features(h, t, wid, is_train=is_train,
                                 formation_imputer=_FORM_IMP, row_imputer=_ROW_IMP)


def build_parallel(paths, is_train, label):
    parts = []
    done = 0
    with ProcessPoolExecutor(max_workers=N_BUILD_WORKERS) as ex:
        for feats in ex.map(_build_one, [(p, is_train) for p in paths], chunksize=4):
            done += 1
            if feats is not None:
                parts.append(feats)
            if done % 100 == 0:
...
def main():
    global _FORM_IMP, _ROW_IMP
    ART.mkdir(parents=True, exist_ok=True)
    MODELS.mkdir(parents=True, exist_ok=True)

    all_h = list(ROOT.rglob("*__horizontal_well.csv"))
    ...
    print(f"train wells: {len(train_paths)}  test wells: {len(test_paths)}", flush=True)

    tr_cache = ART / "train_feats.parquet"
    te_cache = ART / "test_feats.parquet"
    if tr_cache.exists() and te_cache.exists():
        print(">> loading cached feature matrices", flush=True)
        train_df = pd.read_parquet(tr_cache)
        test_df = pd.read_parquet(te_cache)
    else:
        print(">> build PLANE-FIT formation imputer", flush=True)
        _FORM_IMP = FormationPlaneKNN(train_paths)
        print(f"   {len(_FORM_IMP.df)} wells", flush=True)
        print(">> build row-level (X, Y) ANCC imputer", flush=True)
        _ROW_IMP = RowKNN(train_paths)
        print(f"   {len(_ROW_IMP.ancc):,} train rows  maxself={_ROW_IMP.maxself}", flush=True)

        print(f">> build train features (parallel, {N_BUILD_WORKERS} workers)", flush=True)
        train_df = build_parallel(train_paths, is_train=True, label="train")
        print(f"   train shape: {train_df.shape}", flush=True)
        print(">> build test features (parallel)", flush=True)
        test_df = build_parallel(test_paths, is_train=False, label="test")
        print(f"   test shape: {test_df.shape}", flush=True)
        train_df.to_parquet(tr_cache); test_df.to_parquet(te_cache)
        print(f"   cached -> {tr_cache} , {te_cache}", flush=True)
    ...
    # shuffled group folds (sklearn 1.5.2 GroupKFold has no shuffle arg)
    ...
    splits = [(np.where(well_fold != f)[0], np.where(well_fold == f)[0]) for f in range(N_SPLITS)]
    ...
    def train_lgb(seed):
        ...
        oof = np.zeros(len(train_df), np.float32); test_pred = np.zeros(len(test_df), np.float32)
        for fold, (tr, va) in enumerate(splits):
            dtr = lgb.Dataset(train_df.iloc[tr][feature_cols], label=ytr[tr])
            dva = lgb.Dataset(train_df.iloc[va][feature_cols], label=ytr[va], reference=dtr)
            ...
            print(f"   fold {fold}: rmse={np.sqrt(np.mean((oof[va]-ytr[va])**2)):.4f} it={m.best_iteration}", flush=True)
        rmse = float(np.sqrt(np.mean((oof - ytr) ** 2)))
        print(f"   LGB seed={seed}: OOF rmse={rmse:.4f}", flush=True)
        results[f"lgb_{seed}"] = {"oof": oof, "test": test_pred, "rmse": rmse}

    def train_xgb(seed):
        print(f"\n>> XGB(gpu) seed={seed}", flush=True)
        params = dict(XGB_PARAMS); params["seed"] = seed
        oof = np.zeros(len(train_df), np.float32); test_pred = np.zeros(len(test_df), np.float32)
        for fold, (tr, va) in enumerate(splits):
            dtr = xgb.DMatrix(train_df.iloc[tr][feature_cols].values, label=ytr[tr])
            dva = xgb.DMatrix(train_df.iloc[va][feature_cols].values, label=ytr[va])
            m = xgb.train(params, dtr, num_boost_round=5000, evals=[(dva, "val")],
                          early_stopping_rounds=125, verbose_eval=False)
            it = (0, m.best_iteration + 1)
            oof[va] = m.predict(dva, iteration_range=it)
            test_pred += m.predict(xgb.DMatrix(test_df[feature_cols].values), iteration_range=it) / N_SPLITS
            m.save_model(str(MODELS / f"xgb_seed{seed}_fold{fold}.json"))
            print(f"   fold {fold}: rmse={np.sqrt(np.mean((oof[va]-ytr[va])**2)):.4f} it={m.best_iteration}", flush=True)
        rmse = float(np.sqrt(np.mean((oof - ytr) ** 2)))
        print(f"   XGB seed={seed}: OOF rmse={rmse:.4f}", flush=True)
        results[f"xgb_{seed}"] = {"oof": oof, "test": test_pred, "rmse": rmse}

    for s in LGB_SEEDS:
        train_lgb(s)
    train_xgb(42)

    # Ridge stack (non-negative), same as konbu
    from sklearn.linear_model import Ridge
    keys = list(results.keys())
    stack_X = np.column_stack([results[k]["oof"] for k in keys])
    ridge = Ridge(alpha=1.0, fit_intercept=False, positive=True)
    ridge.fit(stack_X, ytr)
    stack_oof = ridge.predict(stack_X)
    rmse_stack = float(np.sqrt(np.mean((stack_oof - ytr) ** 2)))
    weights = (ridge.coef_ / max(ridge.coef_.sum(), 1e-9)).tolist()
    rmse_avg = float(np.sqrt(np.mean((np.mean(stack_X, 1) - ytr) ** 2)))
    print(f"\n=== summary ===", flush=True)
    for k in keys:
        print(f"  {k}: {results[k]['rmse']:.4f}", flush=True)
    print(f"  simple avg : {rmse_avg:.4f}", flush=True)
    print(f"  ridge stack: {rmse_stack:.4f}  weights={dict(zip(keys, np.round(weights,3)))}", flush=True)

    # persist blend recipe + test prediction
    stack_test = ridge.predict(np.column_stack([results[k]["test"] for k in keys]))
    json.dump({"keys": keys, "ridge_coef": ridge.coef_.tolist(),
               "rmse_stack": rmse_stack, "rmse_avg": rmse_avg,
               "per_model": {k: results[k]["rmse"] for k in keys}},
              open(MODELS / "blend.json", "w"), indent=2)
    sub_tvt = test_df["last_known_tvt"].to_numpy(float) + stack_test.astype(float)
    pd.DataFrame({"id": test_df["prediction_id"], "tvt": sub_tvt}).to_csv(ART / "submission_local.csv", index=False)
    print(f">> saved {len(keys)} model families to {MODELS} ; local test sub -> {ART/'submission_local.csv'}", flush=True)
