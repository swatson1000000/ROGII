"""No-UK ensemble retrain (part 2): GPU-CatBoost x3 on base-222 + dip(5) + cwt(4) = 231 feats.

Identical to experiments/ens_train_cat.py EXCEPT the 3 UK cols are excluded from feature_cols (same
exclusion + same column order as ens_nouk_train_lgb.py, so the kernel's feature_cols.json drives both).

Run (GPU CatBoost): nohup python -u experiments/ens_nouk_train_cat.py > log/ens_nouk_cat_$(date +%Y%m%d_%H%M%S).log 2>&1 &
"""
...
print(f">> train {X.shape}  #feats={len(feature_cols)} (UK excluded)", flush=True)
...
print(f"\n=== CatBoost-231 (frontier_ens NO-UK) summary ===", flush=True)
for k, v in results.items():
    print(f"  {k}: {v:.4f}", flush=True)
json.dump(results, open(MODELS / "cat_summary.json", "w"), indent=2)
print("=== ENS NO-UK CAT DONE ===", flush=True)
