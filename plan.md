...
| Item | Value |
|------|-------|
| Best Kaggle LB score | **8.269** (public) — frontier-222 GBM stack + 128-seed likelihood-weighted PF output-blend (w=0.44), OOF 9.17 → **LB 8.269 (LB−OOF = −0.90, FAVORABLE)**. **−1.85 ft vs prior 10.122 — biggest jump of the project.** Now at the published-notebook frontier (~8.2); public board top 5.986. Prior bests: 10.122 (frontier GBM-only), 11.903 (konbu+CatBoost), 11.921, 12.624. (GP anchor regressed to 12.631 — dead.) |
...
| Currently running | nothing. **✅ 2026-06-06 — NEW BEST LB 8.269** (kernel `rogii-frontier-inference` v2 = frontier-222 GBM + 128-seed likelihood-weighted PF output-blend w=0.44; OOF 9.17 → LB 8.269, LB−OOF −0.90 favorable; −1.85 ft vs 10.122, biggest jump of the project). PF run as a runtime-written loky worker module (bit-exact vs pkl). Dataset `rogii-frontier-artifacts` unchanged. Open next: bracket w (w=0.30 second sub to test transfer), push PF further (more seeds / 14-config beam ensemble / per-well selector), or bank+harden. See `experiments/pf_output_blend.py` / `experiments/make_frontier_kernel.py` / PICK_UP_HERE.md. **⚠️ Super-solution reproduction is DONE and FAILED the gate** (combined OOF 10.452 raw / 10.424 +postproc+SG vs frontier 10.356 → ~0.07–0.10 ft WORSE; see 2026-06-04 log entry). Not productionized, nothing pushed. **Open fork:** rule out CatBoost×3 diversity (only cat_42 trained) before declaring super dead, or pivot to the resistivity/ANCC-inversion lever. See PICK_UP_HERE.md. |
