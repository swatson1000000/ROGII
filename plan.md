...
| Currently running | nothing. **✅ 2026-06-08 — NEW BEST LB 8.158** (kernel `rogii-frontier-inference` **v5** = frontier-222 GBM + 128-seed likelihood-weighted PF output-blend at **w=0.57**, the curve vertex). PF mix mapped on 4 LB points (0.44→8.269, **0.57→8.158**, 0.60→8.164, 0.77→8.429): favorable transfer gap widens with w → LB-optimal w≈0.57 > OOF-optimal 0.44; parabola predicted 8.157, got 8.158 (LB low-noise). **0.57 & 0.60 are a tie — keep BOTH v5+v4 as final private-LB selections. The PF blend weight is now FULLY mapped + exhausted (~0.11 ft total was all it held).** Live kernel v5 = banked best. Dataset `rogii-frontier-artifacts` unchanged. **Also 2026-06-07: hunt for a 2nd orthogonal output-blend member is DONE — averaging axis SPENT** (`blend_candidate_scan.py` best +0.027; BUILT 14-config beam ensemble `beam_real_gate.py` standalone 15.7 ≈ null, +0.015 on blend; beam redundant with the PF). **Open next:** the per-well SELECTOR (route PF-vs-GBM by n_eval/z_span — last untested structural piece of the public architecture, no guaranteed payoff), or bank+harden. See `experiments/make_frontier_kernel.py` / PICK_UP_HERE.md. **⚠️ Super-solution reproduction DONE + FAILED the gate** (OOF 10.452 vs frontier 10.356; see 2026-06-04 log). Not productionized. |
