...
| Item | Value |
|------|-------|
| Best Kaggle LB score | **8.164** (public) — frontier-222 GBM stack + 128-seed likelihood-weighted PF output-blend at **w=0.60** (kernel v4), OOF 9.327 → **LB 8.164 (LB−OOF = −1.16)**. **−0.105 vs prior 8.269.** Found by bracketing the PF mix on 3 LB points (0.44→8.269, **0.60→8.164**, 0.77→8.429): the favorable PF transfer gap widens with w (−0.90→−1.16→−1.41), so the LB-optimal w (~0.56–0.60) exceeds the OOF-optimal w (0.44); 0.60 is effectively at the optimum (curve flat, ±~0.01). Now past the published-notebook frontier (~8.2); public board top 5.986. Prior bests: 8.269 (w=0.44), 10.122 (frontier GBM-only), 11.903 (konbu+CatBoost), 11.921, 12.624. (GP anchor regressed to 12.631 — dead.) |
...
| Currently running | nothing. **✅ 2026-06-07 — NEW BEST LB 8.164** (kernel `rogii-frontier-inference` **v4** = frontier-222 GBM + 128-seed likelihood-weighted PF output-blend at **w=0.60**; OOF 9.327 → LB 8.164). Found by bracketing the PF mix on 3 LB points (0.44→8.269, 0.60→8.164, 0.77→8.429): favorable PF transfer gap widens with w, LB-optimal w≈0.56–0.60 > OOF-optimal 0.44; 0.60 is at the flat optimum. **The PF blend weight is now fully explored — done as a lever (~0.1 ft was all it held).** Live kernel v4 = banked best. Dataset `rogii-frontier-artifacts` unchanged. **Also 2026-06-07: hunt for a 2nd orthogonal output-blend member is DONE — averaging axis SPENT** (`blend_candidate_scan.py` best +0.027; BUILT 14-config beam ensemble `beam_real_gate.py` standalone 15.7 ≈ null, +0.015 on blend; beam redundant with the PF). **Open next:** the per-well SELECTOR (route PF-vs-GBM by n_eval/z_span — last untested structural piece of the public architecture, no guaranteed payoff), or bank+harden. See `experiments/make_frontier_kernel.py` / PICK_UP_HERE.md. **⚠️ Super-solution reproduction DONE + FAILED the gate** (OOF 10.452 vs frontier 10.356; see 2026-06-04 log). Not productionized. |
| Primary approach | drift target + formation KNN (plane + row) + PF/beam/NCC + GBM stack; **next = romantamrazov super-solution superset (WLS b_well, frm-RMSE, signal-std, GR envelope/energy/detrend, 4th tw_diff family)** |
...
| 2026-06-07 | **w=0.60 PF mix (NEW BEST)** — same pipeline, blend weight 0.44→0.60; kernel v4 | 9.327 | **8.164** ✅ | **−0.105 vs 8.269.** Landed on the projection (predicted ~8.18). Third point pins the LB(w) curve: smooth parabola, min ~w=0.56–0.60 at ~8.16 → 0.60 is at the optimum. Banked. Live kernel v4 = banked. The PF-mix knob is now exhausted (~0.1 ft was all it held). |
